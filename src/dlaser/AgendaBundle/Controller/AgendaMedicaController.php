...
class AgendaMedicaController extends Controller
{
	...
	public function agendaSedesAction($medico)
	{
            ...
            $profSedes = $medico->getSede();
                        
            $sedes = $this->recursionSedes($userSedes, $profSedes, $key=0, 0);
        ...
        private $array = array();// este array es necesario para almacenar la informacion del las sedes relacionadas 
        public function recursionSedes($userSedes, $profSedes, $y, $x)
        {            
            if($y > (count($userSedes)-1) )            
                return $this->array;            
            elseif($userSedes[$y]->getId() == $profSedes[$x]->getId())
            {
                $this->array[$y] = $userSedes[$y];
                $this->recursionSedes($userSedes, $profSedes, ($y+1), 0);
            }else
                $this->recursionSedes($userSedes, $profSedes, $y, ($x+1));
    ...
    public function saveNewCitaAction()
    {
       ...
       if($agenda && $paciente && $cargo)
       {
            ...
            if($validarCita)
            {
                $response=array("responseCode"=>400, "msg"=>"El paciente cuenta con una cita asignada en esta agenda.");
            }else{
